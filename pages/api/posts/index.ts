...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ApiPaginationResponse<Post> | ApiFailResponse>) {
    if (req.method !== 'GET') {
    ...
    try {
        ...
        const returnedPostPromises: Promise<Post>[] = returnedFileNames.map(async (returnedFileName) => {
