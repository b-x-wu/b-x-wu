...
function FeaturedProject({name, description, imagePath, url}: FeaturedProjectProps) {
  return (
    <Link
      href={url ?? "#"}
      className="relative hover:outline-sky-500 hover:outline-4 h-48 hover:outline group rounded-xl"
    >
      {imagePath != null ? 
        <Image 
          src={imagePath} 
      ...
      <div className="transition-all ease-in-out duration-300 bg-gray-100 bg-opacity-90 absolute xl:w-1/3 xl:inset-y-0 xl:inset-x-auto xl:h-full xl:bottom-auto xl:right-0 md:w-full md:h-9 md:bottom-0 md:inset-x-0 md:inset-y-auto inset-y-0 right-0 w-1/3 flex items-center justify-center group-hover:w-full group-hover:h-full group-hover:rounded-xl rounded-r-lg">
        <div>
